...
(function () {
    ...
    function getGroup(link, sidebar) {
        ...
        while (row && row.parentElement && row !== sidebar) {
    ...
    function saveOrder(parent) {
        ...
        console.log(`[Sidebar Sorter] Saved ${order.length} items`);
    }
    ...
    function applySavedOrder(parent) {
        ...
        const byKey = new Map(rows.map(row => [getRowKey(row), row]));
        const sortedRows = [];
    ...
    function bindRow(row) {
        ...
        row.addEventListener('dragstart', event => {
            ...
            if (event.dataTransfer) {
                ...
                event.dataTransfer.setData('text/plain', getRowKey(row));
            }
        });
        ...
        row.addEventListener('dragend', () => {
            if (draggingParent) saveOrder(draggingParent);
    ...
    function setup() {
        ...
        for (const link of links) {
            ...
            if (group) parents.add(group.parent);
        }

        for (const parent of parents) {
            applySavedOrder(parent);
            getRows(parent).forEach(bindRow);
        }
    }
