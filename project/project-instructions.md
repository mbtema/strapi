...
- Перед действием по существующему GitHub Issue читать его свежие комментарии. Последний комментарий пользователя может уточнять, менять или отменять исходный body; решение принимать по актуальным comments + текущему коду, а не только по заголовку/описанию issue.
...
- Формулировать задачи как инженерную работу: что разработано/автоматизировано/протестировано/проверено, какой был масштаб и какой получен результат. Не сводить работу со скриптами, массовыми выгрузками, batch write и API-валидацией к формулировкам вроде «заполнил» или «перепроверил», если это искажает реальный объём.
