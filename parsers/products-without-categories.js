...
// @version      1.0.2
// @description  Находит активные товары без relation categories
...
(async () => {
  ...
  console.table(rows);
  console.log(`Готово: найдено активных товаров без categories: ${rows.length}`);
  window.productsWithoutCategories = rows;
  downloadCSV(rows, `products_without_categories_${timestamp()}.csv`);
})();
