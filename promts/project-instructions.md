...
- `MutationObserver` ограничивать через `requestAnimationFrame`, debounce/throttle и идемпотентность без потери стабильности.
...
- Структура: `ui-ux/` — UI/UX; `features/` — функции; `parsers/` — проверки; `migrator/` — миграции; `postman/` — API; `promts/` — контекст.
...
- Общий parser регистрировать в `parsers/manifest.json`/Parser Launcher; группу задавать там же через `group`, не дублировать список файлов в Launcher.
...
- В migrator внутри Strapi приоритет `documentId`; `barcode` использовать как cross-system ключ и дополнительную проверку.
...
- Большие массивы оформлять как практичную таблицу/дашборд без лишних полей; Notion relations/rollups/formulas — только если сокращают ручную работу.
